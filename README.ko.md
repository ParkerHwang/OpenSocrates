...
**v1.5.0 릴리스 후보 — 아직 공개 배포하지 않았습니다.** 이 브랜치는 작업별 지원,
...
- 단순 작업은 바로 끝내고 중요한 판단에 필요한 근거와 구조를 보태는 선택적 지원
- Git·일반 폴더의 명시적 로컬 메모리: 제안/승인 상태, 현재 자료 확인,
  체크포인트, 조회·내보내기와 정확한 범위의 삭제
- 기존 구현 재사용, 영향을 받는 호출부와 후속 변경을 살피는 코딩 안내
- URL·버전·읽음 근거를 구분하는 선택적 공식 문서 참조 안내, 읽기 전용 체크포인트
  준비와 범위별 완료 근거 처리. [공식 문서 참조 안내](docs/official-documentation.ko.md)
- 곁가지 질문에 답하고 본래 작업을 이어가며 이미 받은 권한을 재사용하는 영문·한국어 안내

[여섯 상황의 비교](evals/v1.5/practical/RESULTS.md)에서는 정확도가 같았고,
구조화된 메모리 관리가 추가됐으며 영어 승인 질문의 반복이 한 사례에서 줄었습니다.
메모리를 사용한 후보의 작업량은 더 많았습니다. 보편적 품질 우위나 효율 향상을
보장하지 않습니다. 모델별 프로필은 실험 상태이고 기본 지원은 작업 특성에 따라
동작합니다. [메모리 사용·관리 안내](docs/project-memory.ko.md)를 참고하세요.
