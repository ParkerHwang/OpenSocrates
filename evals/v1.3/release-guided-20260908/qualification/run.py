...
SOURCE='95a96a1dfebd62ab225c4424f444ee34cebbbd85'
ZIP_HASH='sha256:5e9c6159d78c237b7b4e3c5b29f67aab767348ec4d381ca07e2ad484dd3ab399'
...
def run(locale,family,arm):
    ...
    profile='candidate' if arm=='J' else 'upfront';work=TEMP/profile/'work'/('qualification-'+cid);work.mkdir()
    observed={}
...
"""
    ...
    try:process.wait(timeout=300)
        ...
        timeout=True;deadline_state={'cli_alive':process.poll() is None,'turn_completed':turn_seen[0],'active_tools':list(active.values())}
    finally:
...
if __name__=='__main__':
    ...
    frozen={'source_commit':SOURCE,'archive_sha256':ZIP_HASH,'protocol_sha256':sha((OUT/'PROTOCOL.md').read_bytes()),'driver_sha256':sha(Path(__file__).read_bytes()),'tasks_sha256':sha(json.dumps(TASKS,sort_keys=True).encode()),'policy_file_sha256':sha((PKG/'content/compiled-response-policy.json').read_bytes()),'frozen_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'maximum_calls':3,'maximum_seconds_per_call':300,'preflight':preflight}
    (OUT/'freeze.json').write_text(json.dumps(frozen,indent=2)+'\n')
    cells=[('ko','changed','J'),('en','forecast','J'),('ko','forecast','J')]
