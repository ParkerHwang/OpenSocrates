...
SOURCE='32d0322c73c8c116b961ddb3a68a75473c715466'
ZIP_HASH='sha256:f51be7c9ce4df606e8df98ed4446377e490d2894c8efaa80ed87e6888044031e'
...
def run(locale,family,arm):
    ...
    profile='candidate' if arm=='J' else 'upfront';work=TEMP/profile/'work'/('native-verification-'+cid);work.mkdir()
    observed={}
...
"""
    ...
    cmd=['codex','exec','--ephemeral','--skip-git-repo-check','--disable','apps','--disable','multi_agent','--model','gpt-5.6-luna','-c','model_reasoning_effort="max"','-c','cli_auth_credentials_store="file"','-c','history.persistence="none"','-c','analytics.enabled=false','-c','web_search="disabled"','-c','sandbox_workspace_write.network_access=false','-c','shell_environment_policy.inherit="all"','--sandbox','workspace-write','--cd',str(work),'--json','-']
    row={'id':cid,'family':family,'locale':locale,'arm':arm,'source_commit':SOURCE,'archive_sha256':ZIP_HASH,'model':'gpt-5.6-luna','effort':'max','cli':'0.145.0','task_sha256':sha(task.encode()),'prompt_sha256':sha(prompt.encode()),'prompt_bytes':len(prompt.encode()),'status':'unavailable','commands':[],'deliveries':[],'application':'unverified','native_observations':[]}
    ...
    captured=[];timing=[];active={};turn_seen=[False];stderr_size=[0];event_types=set()
    controls={name:(PKG/f'skills/{name}/SKILL.md').read_text().strip() for name in ('rigor','trace','opensocrates')}
    def read_events():
        for line in process.stdout:
            try:event=json.loads(line)
            except ValueError:continue
            item=event.get('item',{});kind=item.get('type');typ=event.get('type')
            if kind=='reasoning':continue
            now=round(time.monotonic()-start,3)
            if typ=='turn.completed':turn_seen[0]=True;timing.append({'seconds':now,'event':'turn.completed'})
            if kind:event_types.add(kind)
            if typ in ('item.started','item.completed') and kind not in (None,'agent_message','reasoning'):
                command=item.get('command','');output=item.get('aggregated_output','')
                info={'kind':kind,'native_stream':'--stream' in command and 'decision' in command}
                if typ=='item.started':active[item.get('id')]=info
                else:active.pop(item.get('id'),None)
                timing.append({'seconds':now,'event':typ,**info,'complete_control_references':[name for name,text in controls.items() if text in output]})
            if typ in ('item.started','item.completed','turn.completed','turn.failed','error'):
                captured.append(line)
    def drain_errors():
        for block in iter(lambda:process.stderr.read(4096),''):stderr_size[0]+=len(block.encode())
    reader=threading.Thread(target=read_events,daemon=True);errors=threading.Thread(target=drain_errors,daemon=True);reader.start();errors.start()
    process.stdin.write(prompt);process.stdin.close()
    deadline_state=None
    try:process.wait(timeout=300)
    except subprocess.TimeoutExpired:
        deadline_state={'cli_alive':process.poll() is None,'turn_completed':turn_seen[0],'active_tools':list(active.values())}
        (OUT/'primary-300-marker.json').write_text(json.dumps({'primary_300_result':'deadline','elapsed_seconds':round(time.monotonic()-start,3),'state':deadline_state},indent=2)+'\n')
        try:process.wait(timeout=max(0,600-(time.monotonic()-start)))
        ...
        reader.join(timeout=3);errors.join(timeout=3)
        stage_stop.set()
        if stage_worker:stage_worker.join(timeout=2)
    p=SimpleNamespace(returncode=None if timeout else process.returncode,stdout=''.join(captured),stderr='')
    row['primary_300_result']='deadline' if deadline_state is not None else ('completed' if turn_seen[0] else 'unavailable');row['finite_observation_limit_seconds']=600
    row['evidence_gate']=observed;row['default_deny_environment']=True
    row['timing']=timing;row['turn_completed_observed']=turn_seen[0];row['deadline_state']=deadline_state
    row['observed_item_types']=sorted(event_types);row['stderr_bytes']=stderr_size[0]
    answers=[]
    for line in p.stdout.splitlines():
        ...
        if 'decision codex' in command and isinstance(reply,dict) and reply.get('selector_model_calls')==0:
            literals=[]
            decoder=json.JSONDecoder()
            for match in re.finditer(r'\{',command):
                try:value,_=decoder.raw_decode(command[match.start():])
                except ValueError:continue
                if isinstance(value,dict) and value.get('operation')=='select' and isinstance(value.get('routing'),dict):
                    allowed=set(json.loads((PKG/'skills/opensocrates/references/decision/features.json').read_text())['features'])
                    codes=[f.get('key') for f in value['routing'].get('features',[]) if isinstance(f,dict)]
                    if codes and all(c in allowed for c in codes) and value.get('context')==reply.get('scope',{}).get('context'):
                        literals.append({'closed_feature_codes':codes,'context_matches_reply':True,'source':'JSON literal in observed native invocation command'})
            row['native_observations'].append({'invocation_observed':True,'status':reply.get('status'),'selected':reply.get('selected'),'reason':reply.get('reason'),'applied':reply.get('applied'),'request_literals':literals,'semantic_classification':'agent_supplied_unverified'})
    ...
    row.update(status='deadline' if timeout else ('completed' if p.returncode==0 and answers and turn_seen[0] else 'unavailable'),exit_code=p.returncode,elapsed_seconds=round(time.monotonic()-start,2))
...
if __name__=='__main__':
    ...
    for arm in ('J',):
        ...
        p=subprocess.run(['codex','--disable','apps','--disable','multi_agent','debug','prompt-input','inspection'],env=env,cwd=home,text=True,capture_output=True,timeout=30)
    ...
    frozen={'source_commit':SOURCE,'archive_sha256':ZIP_HASH,'protocol_sha256':sha((OUT/'PROTOCOL.md').read_bytes()),'driver_sha256':sha(Path(__file__).read_bytes()),'tasks_sha256':sha(json.dumps(TASKS,sort_keys=True).encode()),'policy_file_sha256':sha((PKG/'content/compiled-response-policy.json').read_bytes()),'frozen_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'maximum_calls':1,'primary_deadline_seconds':300,'maximum_seconds_per_call':600,'preflight':preflight}
