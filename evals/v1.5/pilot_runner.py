            ...
            tool_events.append(
                {
                    ...
                    "policy_call": "opensocrates" in raw_command or "assistance codex" in raw_command,
                }
            )
    return {
        "thread_id": thread_id,
...
def call_codex(
    freeze: dict[str, Any], fixture: Path, codex_home: Path, model: str,
    ...
    command = prefix + [
        "--json", "--ignore-user-config", "--disable", "hooks", "-m", model,
    ...
    try:
        completed = subprocess.run(
            command, input=prompt, text=True, capture_output=True, cwd=fixture,
            env=env, timeout=freeze["limits"]["per_call_wall_seconds"], check=False,
        )
...
def side_question_observation(message: str) -> dict[str, Any]:
    ...
    rejects_proof = bool(re.search(r"not prove|doesn.t prove|cannot prove|does not guarantee|아니|증명하지|보장하지", lowered))
    rejects_inaccessible = bool(re.search(r"not acceptable|cannot|not sufficient|아니|안 됩니다|충족하지", lowered))
    return {"rejects_proof_heuristic": rejects_proof, "rejects_inaccessible_heuristic": rejects_inaccessible}


def make_prompt(freeze: dict[str, Any], task: dict[str, Any], intervention: str, language: str) -> str:
...
def python_result(fixture: Path, expression: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            cwd=fixture, timeout=10, check=False,
        )
...
def checks_for(task_id: str, fixture: Path, stage: str) -> dict[str, bool]:
    if task_id == "coding":
        source = (fixture / "names.py").read_text(encoding="utf-8")
        try:
            tree = ast.parse(source)
            safe = next(node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "safe_display_name")
            reuse = "clean_name" in {node.id for node in ast.walk(safe) if isinstance(node, ast.Name)}
        except (SyntaxError, StopIteration):
            reuse = False
        return {
            "safe_none": python_result(fixture, "from names import safe_display_name;assert safe_display_name(None)=='Guest'"),
            "safe_blank": python_result(fixture, "from names import safe_display_name;assert safe_display_name('  ')== 'Guest'"),
            "safe_normal": python_result(fixture, "from names import safe_display_name;assert safe_display_name('  ada   lovelace ')== 'Ada Lovelace'"),
            "existing_caller": python_result(fixture, "from names import display_name;assert display_name('  ada   lovelace ')== 'Ada Lovelace'"),
            "reuse_helper": reuse,
        }
    if task_id == "planning":
        try:
    ...
    if task_id == "mechanical":
        return {"exact_file": (fixture / "README.md").read_text(encoding="utf-8") == "# Welcome Guide\n\nRead this note before starting.\n"}
    if task_id == "developer":
        expected = 99 if stage == "initial" else 100
        return {
            f"{stage}_behavior": python_result(fixture, f"from quote import discounted_quote;assert discounted_quote(100,10)=={expected}"),
            "existing_caller": python_result(fixture, "from quote import quote;assert quote(100,10)==110"),
        }
...
def run_cell(freeze: dict[str, Any], lane: str, task_id: str, arm: dict[str, str], language: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="os-v15-pilot-") as temp:
        ...
        after_hashes = {path.name: digest(path.read_bytes()) for path in fixture.iterdir() if path.is_file() and path.suffix in {".py", ".md", ".json"}}
        return {
            "lane": lane,
            "task": task_id,
            "language": language,
            ...
            "deterministic_success": all(final_checks.values()) and all(not call["failure"] for call in calls),
...
def main() -> None:
    ...
    client = subprocess.run([freeze["client_path"], "--version"], capture_output=True, text=True, check=True).stdout.strip()
    ...
    with ThreadPoolExecutor(max_workers=freeze["limits"]["max_parallel_calls"]) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                result = {"lane": args.lane, "task": item[0], "arm": item[1]["id"], "language": item[2], "runner_failure": type(exc).__name__, "deterministic_success": False}
