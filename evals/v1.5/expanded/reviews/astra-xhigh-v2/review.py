...
def materialize(
    ...
    locked = None
    if phase == "evidence":
        lock = read(HERE / "locks/first-pass.lock.json")
        source = HERE / "assessments" / f"{assignment['assignment_id']}-first_pass.json"
        assert sha(source) == lock["assessments"][assignment["assignment_id"]]
...
def lock_phase(manifest: dict, phase: str) -> None:
    ...
    for assignment in manifest["assignment_order"]:
        ...
        assessments[assignment["assignment_id"]] = sha(path)
    name = "first-pass.lock.json" if phase == "first_pass" else "all-ratings.lock.json"
    save(
        HERE / "locks" / name,
        {
            "phase": phase,
...
def main() -> None:
    ...
    parser.add_argument("phase", choices=["first_pass", "evidence"])
    parser.add_argument("--assignment")
    args = parser.parse_args()
    manifest = freeze()
    if args.phase == "evidence":
        assert (HERE / "locks/first-pass.lock.json").exists()
    assignments = [
        row
    ...
    with ThreadPoolExecutor(max_workers=manifest["limits"]["max_parallel_calls"]) as executor:
        futures = [
            executor.submit(run_assignment, manifest, row, args.phase) for row in assignments
        ]
        for future in as_completed(futures):
            result = future.result()
            results.append(result)
            print(json.dumps(result), flush=True)
    if not args.assignment and all(
        result["status"] in {"locked", "already_locked"} for result in results
    ):
