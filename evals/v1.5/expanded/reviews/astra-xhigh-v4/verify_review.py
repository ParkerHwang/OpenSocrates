...
def verify(complete: bool = True) -> dict:  # noqa: C901
    ...
    original_by_packet = {
        ...
        for value in original_first.values()
    ...
    for identifier, reference in references.items():
        ...
        assignment = {
            ...
            "packet_ids": [packet["packet_id"] for packet in value["packets"]],
        }
        validate(
            value,
            schema=schema,
            ...
            locked={
                "packets": [original_by_packet[packet["packet_id"]] for packet in value["packets"]]
            },
        )
        reviewed.extend(p["packet_id"] for p in value["packets"])
    for assignment in manifest["assignment_order"]:
        assert len(assignment["packet_ids"]) == 1
    ...
    return {
        ...
        "maximum_completed_v4_overlap": maximum,
        "human_scores": None,
