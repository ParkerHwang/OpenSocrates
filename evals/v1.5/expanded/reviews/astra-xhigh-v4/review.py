...
CITATION SYNTAX: A citation must identify an existing named JSON field, optionally followed by an artifact line. Bare packet:, deterministic:, packet:/ and deterministic:/ are not accepted by the frozen validator. For an absence claim, cite an existing coverage or limitation field (for example deterministic:/measurement_limit ONLY when that field actually exists). This is syntax guidance, not a requested score or a conclusion about the evidence.
