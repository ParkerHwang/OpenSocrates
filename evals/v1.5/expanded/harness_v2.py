...
def summarize(raw: str) -> tuple[dict[str, Any], str]:  # noqa: C901
    ...
    for identifier in dict.fromkeys([*started, *completed]):
        item = completed.get(identifier, started.get(identifier, {}))
        command = item.get("command", "")
        operations = re.findall(
            r"(?:launch\.sh[\"']?\s+|python\S*\s+-m\s+opensocrates\s+)(memory|assistance|decision)\b",
            command,
        )
        responses = []
...
def selftest() -> None:
    ...
    print("harness-v2-selftest: PASS")
