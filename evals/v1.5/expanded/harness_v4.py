...
def selftest() -> None:
    ...
    for payload in ([{"record_id": "synthetic"}], "text", None, 3):
        observed, _ = summarize(
            json.dumps(
                {
                    "type": "item.completed",
                    "item": {
                        "id": "shape",
                        "type": "command_execution",
                        "command": "bash /p/launch.sh memory codex",
                        "exit_code": 0,
        ...
        assert observed["usage"]["input_tokens"] == 12
        assert observed["structured_status_counts"] == {"ok": 1}
    print("harness-v4-selftest: PASS")
