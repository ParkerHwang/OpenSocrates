...
def cell(freeze: dict, scheduled: dict) -> dict:  # noqa: C901
    ...
    output = HERE / "diagnostic-results-v4" / scheduled["id"]
    ...
    with tempfile.TemporaryDirectory(prefix="os-v15-diag-v4-", dir="/private/tmp") as temporary:
        base = Path(temporary)
...
def main() -> None:
    freeze, commit = harness.verify_freeze(HERE / "diagnostic-freeze.v4.json")
    ...
    previous = json.loads((HERE / "diagnostic-freeze.v3.json").read_text())
    if any(
        not (HERE / "diagnostic-results-v3" / entry["id"] / "result.json").exists()
        for entry in previous["cells"]
    ):
        raise SystemExit("The preceding diagnostic batch is unfinished")
