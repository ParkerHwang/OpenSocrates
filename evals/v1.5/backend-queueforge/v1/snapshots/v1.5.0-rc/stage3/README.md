...
For a migrated version 1 idempotency key, replay accepts the original three-field command when the new scheduling fields are absent. Explicitly supplying any new scheduling field conflicts with that historical command, including a value equal to a version 2 default. New version 2 keys still compare resolved defaults. This preserves the old command boundary without changing stored fingerprints or job state.
...
The final feedback's measured read window lost transport after warmup and its correctness gate failed, so it does not establish useful throughput or a database bottleneck. The existing write path holds one SQLite writer lock while checking admission, inserting jobs, and persisting idempotency; this is needed for cross-process atomicity. No load optimization was made from that inconclusive result.
