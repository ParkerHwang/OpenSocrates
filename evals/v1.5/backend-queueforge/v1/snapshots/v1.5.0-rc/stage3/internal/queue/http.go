...
func resolve(in inputJSON) (Input, bool) {
	...
	return Input{Queue: in.Queue, Payload: p, MaxAttempts: int(max), Priority: int(priority), RunAtMS: runAt, RetryBaseMS: retry, legacyShape: len(in.Priority) == 0 && len(in.RunAtMS) == 0 && len(in.RetryBaseMS) == 0}, true
}
