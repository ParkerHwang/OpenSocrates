...
func main() {
	...
	maxPending := flag.Int("max-pending", 100000, "maximum ready and leased jobs per tenant")
	maxInflight := flag.Int("max-inflight", 100000, "maximum live leases per tenant and queue")
	flag.Parse()
	if !strings.HasPrefix(*addr, "127.0.0.1:") || !strings.HasPrefix(*path, "/") || *maxPending <= 0 || *maxInflight <= 0 {
	...
	store, e := queue.New(*path, clock, *maxPending, *maxInflight)
	if e != nil {
		log.Fatal(e)
	}
	defer store.Close()
