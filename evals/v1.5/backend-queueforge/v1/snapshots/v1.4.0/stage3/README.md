...
For a migrated version 1 idempotency key, replay accepts the original command with its version 1 fields (including an explicitly supplied original `max_attempts` default). Adding any version 2 scheduling field conflicts under that key, even when its value equals a version 2 default. New version 2 keys compare resolved defaults, so omitted and explicit default values replay normally.
...
The stage 2 feedback identified one migrated-key replay failure, which this follow-up fixes. Its preliminary read run served 15,097 successful warmup requests (about 5,032 requests/s at concurrency 1), but the measurement window ended with transport errors and the correctness gate failed. Other load cases were unavailable. Those observations do not establish comparative throughput or a storage bottleneck, so this follow-up makes no speculative SQL or durability change.
