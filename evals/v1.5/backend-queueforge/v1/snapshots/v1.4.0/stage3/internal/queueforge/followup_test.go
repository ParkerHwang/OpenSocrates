...
func TestMigrationAndFutureVersion(t *testing.T) {
	...
	for _, body := range []string{
		...
		`{"queue":"q","payload":{},"run_at_ms":1050}`,
	} {
		st, v = api(t, s, "POST", "/v1/jobs", "key", body)
		mustStatus(t, st, 409, v)
	}
	st, v = api(t, s, "POST", "/v1/jobs", "key", `{"queue":"q","payload":{},"max_attempts":3}`)
	mustStatus(t, st, 200, v)
	st, v = api(t, s, "POST", "/v1/jobs", "new-key", `{"queue":"q","payload":{"new":true}}`)
	mustStatus(t, st, 201, v)
	newID := v["job"].(map[string]any)["id"]
	st, v = api(t, s, "POST", "/v1/jobs", "new-key", `{"queue":"q","payload":{"new":true},"priority":0,"retry_base_ms":1000}`)
	mustStatus(t, st, 200, v)
	if v["job"].(map[string]any)["id"] != newID {
		t.Fatal("version 2 default replay changed resource")
	}
