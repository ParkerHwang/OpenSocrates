...
func (s *Service) submit(w http.ResponseWriter, r *http.Request, tenant string, batch bool) {
	...
	for i, in := range normalized {
		...
		// Version 1 commands had none of these fields. An explicit version 2
		// field changes that command even when its value equals today's default.
		if len(inputs[i].Priority) != 0 || len(inputs[i].RunAtMS) != 0 || len(inputs[i].RetryBaseMS) != 0 {
