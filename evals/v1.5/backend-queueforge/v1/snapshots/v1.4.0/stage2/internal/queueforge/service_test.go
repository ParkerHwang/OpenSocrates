...
func TestQueueLifecycle(t *testing.T) {
	...
	status, v := call("POST", "/v1/jobs", "a", "key", `{"queue":"mail","payload":{"b":2,"a":1},"max_attempts":2,"retry_base_ms":0}`)
	if status != 201 {
		t.Fatal(status, v)
	}
	id := job(v)["id"].(string)
	status, v = call("POST", "/v1/jobs", "a", "key", `{"payload":{"a":1,"b":2},"max_attempts":2,"queue":"mail","retry_base_ms":0}`)
...
func TestSharedDatabaseClaimAndMaxAttempts(t *testing.T) {
	...
	st, v := request(a, "POST", "/v1/jobs/batch", "batch", `{"jobs":[{"queue":"q","payload":{"x":1},"max_attempts":1},{"queue":"q","payload":{},"retry_base_ms":0}]}`)
	if st != 201 {
		t.Fatal(st, v)
	}
	jobs := v["jobs"].([]any)
