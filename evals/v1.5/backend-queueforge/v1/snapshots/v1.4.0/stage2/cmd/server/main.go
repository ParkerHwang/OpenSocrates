...
func main() {
	...
	maxPending := flag.String("max-pending", "100000", "tenant pending job limit")
	maxInflight := flag.String("max-inflight", "100000", "tenant queue inflight lease limit")
	flag.Parse()
	pending, ep := strconv.Atoi(*maxPending)
	inflight, ei := strconv.Atoi(*maxInflight)
	if ep != nil || ei != nil || pending <= 0 || inflight <= 0 {
		fmt.Fprintln(os.Stderr, "limits must be positive integers")
		os.Exit(2)
	}
	...
	s, e := queueforge.OpenWithLimits(*dbPath, clock, pending, inflight)
	if e != nil {
