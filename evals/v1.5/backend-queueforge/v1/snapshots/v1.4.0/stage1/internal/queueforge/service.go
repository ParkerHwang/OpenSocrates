...
const jobCols = `id,tenant,queue,payload,state,attempts,max_attempts,seq,created_at_ms,available_at_ms,lease_until_ms,last_error,result`
...
func getJob(ctx context.Context, q rowQuerier, tenant, id string) (Job, error) {
	...
	e := q.QueryRowContext(ctx, `SELECT `+jobCols+` FROM jobs WHERE tenant=? AND id=?`, tenant, id).Scan(&j.ID, &j.Tenant, &j.Queue, &p, &j.State, &j.Attempts, &j.MaxAttempts, &j.CreatedSeq, &j.CreatedAtMS, &j.AvailableAtMS, &lease, &last, &res)
	if e != nil {
		return j, e
	}
...
func normalize(in Input) (commandInput, *APIError) {
	...
	return commandInput{in.Queue, p, n}, nil
}
...
func (s *Service) submit(w http.ResponseWriter, r *http.Request, tenant string, batch bool) {
	...
	x = s.write(r.Context(), func(c *sql.Conn) *APIError {
		...
		for i, in := range normalized {
			...
			ids[i] = id
			_, e = c.ExecContext(r.Context(), `INSERT INTO jobs(id,tenant,queue,payload,state,attempts,max_attempts,created_at_ms,available_at_ms) VALUES(?,?,?,?, 'ready',0,?,?,?)`, id, tenant, in.Queue, string(in.Payload), in.MaxAttempts, now, now)
			if e != nil {
				return storage()
			}
			j, e := getJob(r.Context(), c, tenant, id)
...
func (s *Service) claim(w http.ResponseWriter, r *http.Request, tenant, queue string) {
	...
	x := s.write(r.Context(), func(c *sql.Conn) *APIError {
		_, e := c.ExecContext(r.Context(), `UPDATE jobs SET state=CASE WHEN attempts>=max_attempts THEN 'dead' ELSE 'ready' END, available_at_ms=?, lease_until_ms=NULL, lease_token=NULL, last_error='lease_expired' WHERE tenant=? AND queue=? AND state='leased' AND lease_until_ms<=?`, now, tenant, queue, now)
		if e != nil {
			return storage()
		}
		var id string
		e = c.QueryRowContext(r.Context(), `SELECT id FROM jobs WHERE tenant=? AND queue=? AND state='ready' AND available_at_ms<=? ORDER BY seq LIMIT 1`, tenant, queue, now).Scan(&id)
		if e == sql.ErrNoRows {
			return nil
		}
		if e != nil {
...
func (s *Service) lifecycle(w http.ResponseWriter, r *http.Request, tenant, id, op string) {
	...
	x := s.write(r.Context(), func(c *sql.Conn) *APIError {
		...
		case "fail":
			...
			_, e = c.ExecContext(r.Context(), `UPDATE jobs SET state=?,available_at_ms=?,last_error=?,lease_token=NULL,lease_until_ms=NULL WHERE tenant=? AND id=?`, state, now, req.Error, tenant, id)
		}
		if e != nil {
			return storage()
		}
...
func (s *Service) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case len(path) == 4 && path[0] == "v1" && path[1] == "jobs" && r.Method == "POST" && (path[3] == "heartbeat" || path[3] == "complete" || path[3] == "fail"):
