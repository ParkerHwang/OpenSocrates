...
func main() {
	...
	maxInflight := flag.String("max-inflight", "100000", "tenant queue active lease limit")
	flag.Parse()
	pending, pe := strconv.Atoi(*maxPending)
	inflight, ie := strconv.Atoi(*maxInflight)
	if pe != nil || ie != nil || pending <= 0 || inflight <= 0 {
		log.Fatal("capacity limits must be positive integers")
	}
	if *path == "" || !isAbsolute(*path) {
		log.Fatal("--db must be an absolute path")
	}
	store, e := queue.OpenWithLimits(*path, pending, inflight)
	if e != nil {
