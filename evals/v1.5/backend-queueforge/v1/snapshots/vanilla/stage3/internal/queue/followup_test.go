...
func TestVersionOneMigrationAndFutureGuard(t *testing.T) {
	...
	server := httptest.NewServer(&Server{Store: b})
	defer server.Close()
	for _, tc := range []struct {
		body string
		want int
	}{
		{`{"queue":"q","payload":{}}`, http.StatusOK},
		{`{"queue":"q","payload":{},"retry_base_ms":0}`, http.StatusOK},
		{`{"queue":"q","payload":{},"retry_base_ms":1000}`, http.StatusConflict},
	} {
		req, e := http.NewRequest(http.MethodPost, server.URL+"/v1/jobs", bytes.NewBufferString(tc.body))
		if e != nil {
			t.Fatal(e)
		}
		req.Header.Set("X-Tenant-ID", "t")
		req.Header.Set("Idempotency-Key", "old")
		...
		if res.StatusCode != tc.want {
			t.Fatalf("legacy replay body %s: got %d, want %d", tc.body, res.StatusCode, tc.want)
		}
	}
