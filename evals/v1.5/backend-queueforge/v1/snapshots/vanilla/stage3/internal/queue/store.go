...
func (s *Store) Submit(ctx context.Context, tenant, endpoint, key, command string, inputs []Input, now int64) ([]Job, bool, error) {
	...
	e := s.write(ctx, func(c *sql.Conn) error {
		...
		if err == nil {
			if old != command {
				// A v1 command lacked scheduling fields. An omitted retry base in
				// the original request can replay despite the new default; an
				// explicit retry base changes that historical command unless zero.
				var legacy []struct {
					Queue       string          `json:"queue"`
					Payload     json.RawMessage `json:"payload"`
					MaxAttempts int             `json:"max_attempts"`
				...
				for n, in := range inputs {
					if in.Queue != legacy[n].Queue || string(in.Payload) != string(legacy[n].Payload) || in.MaxAttempts != legacy[n].MaxAttempts || in.Priority != 0 || in.RunAtSet || (in.RetryBaseSet && in.RetryBaseMS != 0) {
						return ErrIdem
					}
				}
			}
