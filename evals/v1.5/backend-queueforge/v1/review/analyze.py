...
def summarize():
    ...
    for stage in (2, 3):
        path = ROOT / "evidence/performance-v2" / f"stage{stage}" / "summary.json"
        if not path.exists():
            continue
    ...
    return {
        ...
        "superseded_meter_evidence": {
            "executed_invalid_cells": 6,
            "planned_skipped_rows": 93,
            "path": "evidence/performance-stage2 and performance-stage3",
            "reason": "Original closed-loop drain timer cancelled requests at the warmup boundary. No RPS estimate is interpreted from these observations.",
...
def main():
    ...
    if result["cells"]:
        with (OUT / "performance-cells.csv").open("w", newline="") as stream:
            writer = csv.DictWriter(stream, fieldnames=list(result["cells"][0]))
            writer.writeheader()
