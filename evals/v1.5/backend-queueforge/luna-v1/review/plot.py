...
base_labels = ["Vanilla Codex", "OpenSocrates 1.4", "OpenSocrates 1.5 RC"]
gates = {a: next(x for x in data["stages"] if x["arm"] == a and x["stage"] == 3) for a in arms}
qualified = {a: all(gates[a].get(k) is True for k in ("api_checks_pass", "own_tests_pass", "dependency_locks_unchanged")) for a in arms}
labels = [label + ("" if qualified[a] else "*") for a, label in zip(arms, base_labels)]
empty = {"median": float("nan"), "min": float("nan"), "max": float("nan"), "n": 0}
...
for ax, workload, metric, title, ylabel in plots:
    for arm, label, color in zip(arms, labels, colors):
        stats = [(group(arm, workload, n, 0)[metric] or empty) for n in (1, 16, 64)]
        y = np.array([s["median"] for s in stats])
        errors = [y - np.array([s["min"] for s in stats]), np.array([s["max"] for s in stats]) - y]
        ax.errorbar([0, 1, 2], y, yerr=errors, color=color, label=label, marker="o", capsize=4, linewidth=2, linestyle="--" if not qualified[arm] else "-")
    ax.set(xticks=[0, 1, 2], xticklabels=["1", "16", "64"], xlabel="Concurrent clients", ylabel=ylabel, title=title, ylim=(0, None))
    ax.grid(axis="y", alpha=.15)
ax = axes[1, 0]
for arm, label, color in zip(arms, labels, colors):
    stats = [(group(arm, "read", 1, rate)["scheduled_p99_ms"] or empty) for rate in (100, 500, 1000)]
    y = np.array([s["median"] for s in stats])
    ax.errorbar([0, 1, 2], y, yerr=[y - np.array([s["min"] for s in stats]), np.array([s["max"] for s in stats]) - y], color=color, marker="o", capsize=4, linewidth=2, linestyle="--" if not qualified[arm] else "-")
...
for i, (arm, color) in enumerate(zip(arms, colors)):
    y = [(group(arm, work, 64, 0)["server_peak_rss_mib"] or empty)["median"] for work in ("read", "lifecycle")]
    ax.bar(np.arange(2) + (i - 1) * .24, y, width=.22, color=color, hatch="//" if not qualified[arm] else None)
...
for ax in axes:
    ...
    ax.grid(axis="y", alpha=.15)
fig.suptitle("Luna — three fresh development sessions per condition", x=.06, ha="left", fontsize=18, weight="bold")
fig.text(.06, .035, "Same requested gpt-6-luna / medium / codex-cli 0.158.0-alpha.2. Nine calls maximum; no outcome retries.\nThese are observed work measures, not billed cost or a causal efficiency estimate. No new preparation-agent calls.\nAsterisks in the performance figure identify failed artifact gates. These figures do not isolate a causal model/version effect.", fontsize=9, color="#444444")
