# Read and reproduce the QueueForge Luna evidence
...
Run `go test -race ./...` in its stage-3 directory. Consult REPORT.md for the recorded result of each original artifact. A passing
suite can contain only the supplied driver test; inspect domain-test coverage and
retained process scripts separately. Do not change original artifacts to make an
old result pass. Downloading the pinned public modules may
...
```sh
(cd benchmark/loadgen && go test -race ./... && go build -o loadgen . && ./loadgen --selfcheck)
python3 -m unittest discover -s benchmark-v2 -p 'test_*.py' -v
```

`benchmark/runner.py` exposes `prepare_seed` and `run_cell`. For a new measurement,
...
`$BUNDLED_CODEX` and `$USER_HOME` placeholders, so they are provenance, not directly runnable model
setup scripts in this portable export.

`summary.json` and `performance-cells.csv` retain all corrected cells and gate
states. Luna uses the corrected meter prospectively. The inherited benchmark/REPAIR.md and VALIDATION.json describe the earlier Sol meter repair, not a new failure or rerun in this Luna study. Full raw timing
