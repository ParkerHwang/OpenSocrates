...
def verify(manifest, digest):
    ...
    assert frozen["model"] == "gpt-6-luna" and frozen["effort"] == "medium"
...
def measure(manifest, states, stage):
    ...
    for name, state in states.items():
        ...
        if not binary:
            stopped[name] = "server binary unavailable"
            save(
        ...
        try:
            ...
            stopped[name] = f"seed preparation unavailable: {error}"
            save(output / name / "seed-unavailable.json", {"reason": str(error)})
    for cell in manifest["performance_cells"][str(stage)]:
        name = cell["arm"]
        if name not in seeds:
            rows.append({"config": cell, "attempted": False, "unavailable_reason": stopped.get(name, "seed unavailable")})
            continue
        if (
            ...
            rows.append(
                {"config": cell, "attempted": False, "unavailable_reason": "frozen global time/data cap"}
            )
            continue
        ...
        emit({"event": "load-finish", "stage": stage, "arm": name, "cell": cell["index"], "rps": measured.get("successful_rps"), "jobs_per_second": measured.get("completed_jobs_per_second"), "errors": measured.get("errors"), "unavailable": result.get("unavailable_reason")})
        if (
            result.get("unavailable_reason")
            or result.get("stopped_transport_failures")
            or result.get("drain_budget_exceeded")
        ):
            stopped[name] = f"Stopped after cell {cell['index']}: " + (result.get("unavailable_reason") or "repeated transport failure or drain budget exceeded")
            del seeds[
                name
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            for stage in [1, 2, 3]:
                ...
                pending = order
                if stage == 1:
                    # The first planned development call also checks account/client
                    # access. It is not an extra probe or a substituted task.
                    invoke(manifest, states[order[0]], stage)
                    if states[order[0]].get("access_blocked"):
                        for name in order[1:]:
                            states[name]["access_blocked"] = True
                    pending = order[1:]
