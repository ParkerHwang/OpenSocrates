...
func Open(path string, c Clock) (*Store, error) {
	...
	// SQLite permits one writer at a time, and busy_timeout is connection-local.
	// Keeping one connection ensures every operation uses the timeout configured
	// above instead of allowing newly-created pooled connections to fail fast.
	db.SetMaxOpenConns(1)
...
func legacyEquivalent(old []byte, in []Input) bool {
	...
	for i := range xs {
		...
		if queue != b.Queue || string(ob) != string(nb) || max != b.Max || priority != b.Priority || int64(run) != b.RunAt || (xs[i]["retry_base_ms"] == nil && b.RetryBase != 0) || (xs[i]["retry_base_ms"] != nil && retry != b.RetryBase) {
			return false
		}
	}
	return true
