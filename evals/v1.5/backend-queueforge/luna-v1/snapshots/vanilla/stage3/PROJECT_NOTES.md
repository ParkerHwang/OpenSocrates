...
- Follow-up feedback exposed `max_attempts: 0` being confused with an omitted field; parsing now preserves presence so only omission gets the default. V1 replay comparison now uses the migrated retry default of 0 when the old command omitted that field. SQLite pool size is one so the connection-local busy timeout applies to every operation; WAL and synchronous=FULL remain unchanged.
- Added focused validation/atomic-batch, shared-file concurrent store, and v1 replay-default tests. Current follow-up verification: `go test -race ./...` and `go build -o .owned-temp/queueforge-followup ./cmd/server` passed; module lock diff is empty. The prior full load measurements were diagnostic under failed correctness gates; no full workload rerun was performed. Remaining limit: no new full multi-process API/load suite was run in this follow-up.
