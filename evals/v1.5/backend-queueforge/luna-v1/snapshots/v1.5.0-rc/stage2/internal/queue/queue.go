		...
		if !match {
			...
			if json.Unmarshal(old, &previous) == nil && len(previous) == len(norm) {
				...
				for i := range previous {
					...
					if x != nil || p.Queue != norm[i].Queue || string(p.Payload) != string(norm[i].Payload) || p.MaxAttempts != norm[i].MaxAttempts || norm[i].Priority != 0 || norm[i].RunAt != 0 || norm[i].RetryBase != 1000 {
						match = false
						break
					}
				}
