...
// UnmarshalJSON rejects null for supplied scalar values. encoding/json otherwise
// silently treats null as the zero value for strings and numbers, which can
// turn malformed commands into valid commands after defaults are applied.
func (in *input) UnmarshalJSON(data []byte) error {
	var fields map[string]json.RawMessage
	if err := json.Unmarshal(data, &fields); err != nil {
		return err
	}
	for _, name := range []string{"queue", "payload", "max_attempts", "priority", "run_at_ms", "retry_base_ms"} {
...
func legacyCommandMatches(old, current []byte) bool {
	...
	for i := range a {
		...
		if a[i].Queue != b[i].Queue || string(compact(a[i].Payload)) != string(compact(b[i].Payload)) || legacyMax != b[i].Max || a[i].Priority != b[i].Priority {
			return false
		}
