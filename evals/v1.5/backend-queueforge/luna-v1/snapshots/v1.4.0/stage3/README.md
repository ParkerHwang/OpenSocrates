...
base zero.
Explicit JSON `null` is rejected for supplied input fields; omitted optional
numeric fields continue to use their documented defaults. Migrated legacy
idempotency commands resolve the old omitted `max_attempts` value to 3 before
comparison.
