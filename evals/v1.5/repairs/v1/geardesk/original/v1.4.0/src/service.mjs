...
import {BusinessError,emptyStore,validateStore,validateCatalog,availability,quote,command,batch,report} from './domain.mjs';
...
export function createService({dataDir,catalogPath,now=()=>new Date().toISOString()}) {
  ...
  async function execute(operation,request={}) {
    let revision=0;
    try {
      if(operation==='command'||operation==='batch') return await locked(async()=>{
        const current=await store(); revision=current.revision;
        const c=await catalog();
        const outcome=(operation==='batch'?batch:command)(current,c,request,now());
