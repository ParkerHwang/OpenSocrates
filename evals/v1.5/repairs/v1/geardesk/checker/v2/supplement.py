...
def run(project, arm, baseline, output):
    ...
    if arm == 'v1.4.0':
        def known_native(h):
            h.reserve('cli','known',lines=[dict(itemId='camera',quantity=1),dict(itemId='tripod',quantity=1)])
            h.good_command('cli',dict(type='checkout',id='known'))
            h.catalog['items'][0]['deposit']=7777;h.catalog['items'][1]['deposit']=8888;h.write_catalog()
            content=dict(type='return.partial',id='known',returnedOn='2026-11-09',lines=[dict(itemId='camera',quantity=1)])
            r=h.good_command('cli',content,key='known-return')['data']
            require(r['refund']==5000 and h.ok('cli','report')['data']['depositHeld']==1000,'known original line amounts 5000/1000')
            replay=h.command('http',content,revision=0,key='known-return')
            require(replay['ok'] and replay['data']==r,'known line replay')
            require(h.good_command('http',dict(type='return',id='known'))['data']['refund']==1000,'known remaining full return')
        scenario('known native booking 5000 refund / 1000 remaining',known_native)
    for fixture_arm in (arm,):
    ...
    summary=dict(label='new-checker-original' if project.parent.name=='apps' and 'repairs' not in str(project) else 'new-checker-derivative',project=str(project),counts={s:sum(c['status']==s for c in checks) for s in ('pass','fail','error','unassessable')},checks=checks,receipts=receipts,limitations=['Known native v1.4 new booking and genuine historical v1.5 quote-line allocation are separate evidence cases; foreign historical allocation import is outside minimum repair.','Output alias failures concern mutable result ownership, not input purity or determinism.','No model judgments collected; historical counts unchanged.'])
