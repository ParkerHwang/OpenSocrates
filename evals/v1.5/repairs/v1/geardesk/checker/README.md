...
5000/1000 facts; both consumers are checked against those retained fields with
changed current catalog. No historical allocation is synthesized. v1.5 is not
required to import the foreign v1.4 aggregate-only schema, which is outside its
original storage format. The v1.4 known-line case covers a compatible retained
historical evidence extension, and may fail until that evidence is consumed.

Usage (paths may be absolute):

```sh
python3 repairs/v1/checker/controls.py
python3 repairs/v1/checker/checks.py --project apps/v1.4.0 --stage 3 --legacy-store evidence/v1.4.0/stage2/legacy-v1-store.json --output repairs/v1/checker-evidence/original-v1.4-main.json
python3 repairs/v1/checker/supplement.py --project apps/v1.4.0 --arm v1.4.0 --baseline . --output repairs/v1/checker-evidence/original-v1.4-supplement.json
```
