...
import {emptyStore,validateCatalog,validateStore,quote,availability,command,batch,report,BusinessError} from '../src/domain.mjs';
...
test('capacity counts simultaneous days and state changes release it',()=>{
  ...
  assert.deepEqual(report(returned.store),{reservationCount:3,counts:{reserved:1,checked_out:0,returned:1,cancelled:1},rentalRevenue:151,depositHeld:50,refunded:50,lateFees:0,auditCount:6,maintenanceCount:0});
});
...
test('partial returns release units and use frozen deposits and fee rate',()=>{
  const priced=validateCatalog({items:[{id:'a',name:'A',stock:3,rate:10,deposit:500}],members:[{id:'m',name:'M',discountBps:0}],pricing:{weekendBps:10000,lateFeePerUnitDay:100}});
  const req={...base,lines:[{itemId:'a',quantity:3}]};
  let s=command(emptyStore(),priced,{type:'reserve',id:'r',...req,idempotencyKey:'r',expectedRevision:0},'fixed').store;
  priced.items[0].deposit=999; priced.pricing.lateFeePerUnitDay=400;
  s=command(s,priced,{type:'checkout',id:'r',idempotencyKey:'c',expectedRevision:1},'fixed').store;
  const first=command(s,priced,{type:'return.partial',id:'r',returnedOn:'2026-11-05',lines:[{itemId:'a',quantity:2}],idempotencyKey:'p',expectedRevision:2},'fixed');
  assert.deepEqual([first.data.refund,first.data.lateFee,first.data.returnedQuantity],[600,400,2]);
  assert.equal(first.data.status,'checked_out');
  assert.equal(availability(first.store,priced,req)[0].available,2);
  assert.deepEqual([report(first.store).depositHeld,report(first.store).refunded,report(first.store).lateFees],[500,600,400]);
  assert.equal(command(first.store,priced,{type:'return.partial',id:'r',returnedOn:'2026-11-05',lines:[{itemId:'a',quantity:2}],idempotencyKey:'p',expectedRevision:2},'later').replay,true);
  code(()=>command(first.store,priced,{type:'return.partial',id:'r',returnedOn:'2026-11-05',lines:[{itemId:'a',quantity:2}],idempotencyKey:'bad',expectedRevision:3},'fixed'),'VALIDATION');
  const last=command(first.store,priced,{type:'return',id:'r',returnedOn:'2026-11-04',idempotencyKey:'end',expectedRevision:3},'fixed');
  assert.deepEqual([last.data.refund,last.data.lateFee,last.data.returnedQuantity],[400,100,1]);
  assert.deepEqual([report(last.store).depositHeld,report(last.store).refunded,report(last.store).lateFees],[0,1000,500]);
});

test('batch composes the same transitions and rolls back failed suffix',()=>{
  const input={expectedRevision:0,idempotencyKey:'batch',commands:[{type:'reserve',id:'r',...base},{type:'checkout',id:'r'},{type:'return.partial',id:'r',returnedOn:'2026-11-03',lines:[{itemId:'a',quantity:1}]}]};
  const result=batch(emptyStore(),catalog,input,'fixed');
  assert.equal(result.store.revision,3); assert.deepEqual(result.store.audit.map(a=>a.type),input.commands.map(c=>c.type));
  assert.equal(result.data[2].refund,50);
  assert.equal(batch(result.store,catalog,input,'later').replay,true);
  code(()=>batch(result.store,catalog,{...input,commands:[{type:'cancel',id:'r'}]},'fixed'),'IDEMPOTENCY_CONFLICT');
  const fresh=emptyStore();
  code(()=>batch(fresh,catalog,{...input,commands:[...input.commands,{type:'cancel',id:'r'}]},'fixed'),'INVALID_TRANSITION');
  assert.equal(fresh.revision,0); assert.deepEqual(fresh.audit,[]);
});
