...
import {DomainError,emptyStore,validateStore,validateCatalog,quote,availability,command,batch,reservations,maintenance,report} from './domain.mjs';
    ...
    try {
      if(operation==='command'||operation==='batch') {
        ...
        try {
          ...
          const current=await catalog();const outcome=(operation==='batch'?batch:command)(current,store,request,{timestamp:now()});
