...
function example(kind) {
  $('operation').value=kind==='maintenance.add'?'command':kind;
  const mutation=kind==='maintenance.add'
    ?{type:kind,id:`maintenance-${crypto.randomUUID().slice(0,8)}`,itemId:'camera',start:'2026-11-10',end:'2026-11-12',quantity:1,idempotencyKey:crypto.randomUUID(),expectedRevision:revision}
    :{type:'reserve',id:`booking-${crypto.randomUUID().slice(0,8)}`,...quote,idempotencyKey:crypto.randomUUID(),expectedRevision:revision};
  $('request').value=JSON.stringify(kind==='quote'?quote:mutation,null,2);
}
