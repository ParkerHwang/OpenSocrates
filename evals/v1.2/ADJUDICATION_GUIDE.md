...
This guide is the single annotation contract for the #65 label adjudication.
Its SHA-256 is recorded in `evals/v1.2/adjudication-freeze-v1.0.0.json` and in
every blind packet before the first case decision is made. Amending this guide
after a decision has been locked requires a new guide version and a new
decision-file version; existing decisions are never edited in place.
...
1. A person or agent who has seen #65 case-level outputs or aggregates cannot
...
aggregates. Blind packets contain no field named `selected`, `intervene`,
`instructions`, `rationale`, `score`, `pass`, `failure`, or `effort`.
...
- `evals/v1.2/adjudication-manifest-v1.0.0.json` — counts, reviewer IDs,
  hashes of guide/packets/decision files, timestamps, git revision, and the
  unchanged raw-result baseline hashes.

Validation is `tools/check_v12_adjudication.py`; it must pass before the
blind is lifted, and only after the manifest is written may historical
results be compared. Post-adjudication numbers are reported beside — never in
place of — the historical schema 1.0.0 results.
