...
This branch is an unpublished candidate. Do not use `opensocrates@1.4.0` from npm until publication. Windows support means native Windows processes, without WSL. Draft [PR #93](https://github.com/ParkerHwang/OpenSocrates/pull/93) tracks `codex/windows-v1.4.0`. The independent-review baseline and first CI run used `9e5ec5bd458e2d8c2b772b6074df25e0192d413e` and failed. The remediation implementation checkpoint `923ad2d020c2081e3aa29af67ac0a32e5dd2238e` passed the five product/platform jobs in [run 34388190521](https://github.com/ParkerHwang/OpenSocrates/actions/runs/34388190521), while Repository governance for the same SHA passed in [run 34388190218](https://github.com/ParkerHwang/OpenSocrates/actions/runs/34388190218). Its artifacts remain evidence for that exact checkpoint only; consult the PR for the branch's current exact head and checks.
...
The Windows workflow runs the native checks/build on `windows-2025`; existing Linux and Apple-silicon jobs remain. Release publication waits for the Windows job and verifies transported Windows checksums before including the new assets. The [first remote CI run](https://github.com/ParkerHwang/OpenSocrates/actions/runs/34316264162) did execute for commit `9e5ec5bd458e2d8c2b772b6074df25e0192d413e` and failed in Product contracts, GitHub and npx installer, and Native Windows x64. The exact remediation checkpoint `923ad2d020c2081e3aa29af67ac0a32e5dd2238e` later completed [run 34388190521](https://github.com/ParkerHwang/OpenSocrates/actions/runs/34388190521) with Python quality, Product contracts, GitHub and npx installer, Native Windows x64, and Native package (darwin-arm64) all successful; Repository governance passed separately in [run 34388190218](https://github.com/ParkerHwang/OpenSocrates/actions/runs/34388190218). The hosted-runner evidence remains separate from earlier private Windows laptop evidence and does not establish live host delivery.

Evidence and outstanding local full-suite failures are recorded in [the working record](windows-v1.4.0-worklog.md). POSIX fixture failures and unavailable authenticated checks must not be reported as passes. macOS was not executed on this laptop; the darwin-arm64 result above is a separate GitHub-hosted runner result.
