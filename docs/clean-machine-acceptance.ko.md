...
이 절차는 출시 전 후보 검증입니다. 후보 버전은 아직 공개 npm 및 GitHub Release
경로에 없을 수 있으므로, 도구가 현재 Pull Request의 정확한 커밋에서 성공한 macOS
CI 패키지 산출물을 받고 실제 npm 설치 프로그램에 전달합니다. 패키지, 설치
프로그램, 두 호스트 트랜잭션, 등록, 관리형 파일 구조와 상태 계약을 검증합니다.
최종 공개 레지스트리 및 릴리스 다운로드 경로까지 증명하지는 않으므로, 출시 후에는
공개된 한 줄 설치 명령도 별도로 실행해야 합니다.
