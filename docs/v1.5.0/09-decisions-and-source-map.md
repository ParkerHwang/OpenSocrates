...
| I08 | Internal SQLite schema 2 adds a migration audit. Existing enrolled schema-1 stores upgrade on the next explicit memory command. A fixed owner-only backup and manifest are checked before the transactional schema change and retained seven days after verification. Delete/prune remove the backup first so a retained copy cannot revive deleted content. | Disposable interruption, expiry, and deletion fixtures pass on macOS. Native frozen and Windows checks remain required; automatic restore is intentionally excluded because later writes could be lost. |
| I09 | A linked worktree must have reciprocal Git pointer metadata: its `.git` file points to its Git directory and that directory's `gitdir` points back to the same file identity. This rejects a copied worktree that otherwise appeared to share the original common directory. | Disposable POSIX identity fixture passes; hosted Windows linked-worktree, reparse, and replacement checks remain required. |
